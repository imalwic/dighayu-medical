...
import { 
  LuMessageSquare, LuDownload, LuReceipt, LuPill, LuCalendar, 
  LuUsers, LuChartBar, LuSmartphone, LuCalendarDays, LuSun, 
  LuMoon, LuUser, LuTrash, LuCircle, LuUmbrella 
} from "react-icons/lu";
    ...
    <div className="min-h-screen bg-[#F8FAFC] font-sans text-slate-800 flex flex-col">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 pt-24 md:pt-28 pb-4 flex-1 w-full">
             ...
             {userRole === 'admin' && (
                 <div className="bg-white px-2 py-2 pr-4 rounded-full border border-slate-200 shadow-sm flex items-center gap-4">
                    ...
                    <Link href="/admin/messages" className="relative group">
                        <div className="flex flex-col"><p className="text-[10px] font-bold text-slate-400 uppercase leading-none mb-0.5">Inbox</p><div className="flex items-center gap-1 text-blue-600"><span className="font-bold text-sm">Chats</span><span className="text-lg"><LuMessageSquare className="w-4 h-4" /></span></div></div>
        ...
        <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-8 md:mb-12`}>
                ...
                <>
                    <div className="bg-white p-5 md:p-6 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 relative overflow-hidden group hover:-translate-y-1 transition-all duration-300">
                        ...
                        <div className="relative z-10">
                            ...
                            <button onClick={downloadDailyReport} className="mt-4 text-xs font-bold text-green-600 bg-green-50 px-3 py-1.5 rounded-lg hover:bg-green-100 transition flex items-center gap-1"><LuDownload className="w-3 h-3"/> Download Report</button>
                        </div>
                    </div>
            ...
            >
                ...
                <div className="space-y-2">
                    <div className={`flex justify-between items-center p-1.5 rounded-lg ${expiredCount > 0 ? 'bg-red-100 text-red-700' : 'bg-slate-50 text-slate-400 opacity-60'}`}><span className="text-xs font-bold flex items-center gap-2"><LuCircle className="w-2.5 h-2.5 fill-current text-red-500" /> Expired</span><span className="text-lg font-black">{expiredCount}</span></div>
                    <div className={`flex justify-between items-center p-1.5 rounded-lg ${expiringSoonCount > 0 ? 'bg-orange-100 text-orange-700' : 'bg-slate-50 text-slate-400 opacity-60'}`}><span className="text-xs font-bold flex items-center gap-2"><LuCircle className="w-2.5 h-2.5 fill-current text-orange-500" /> Soon</span><span className="text-lg font-black">{expiringSoonCount}</span></div>
                    <div className={`flex justify-between items-center p-1.5 rounded-lg ${lowStockCount > 0 ? 'bg-yellow-100 text-yellow-700' : 'bg-slate-50 text-slate-400 opacity-60'}`}><span className="text-xs font-bold flex items-center gap-2"><LuCircle className="w-2.5 h-2.5 fill-current text-yellow-500" /> Low</span><span className="text-lg font-black">{lowStockCount}</span></div>
                </div>
            </div>
        </div>
        ...
        <div className={`grid grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-6 gap-3 md:gap-5 mb-8 md:mb-12`}>
            ...
            {/* 1. New Bill */}
            <Link href="/billing" className="bg-blue-50 border border-blue-100 p-4 md:p-6 rounded-3xl shadow-sm hover:shadow-md hover:bg-blue-100 hover:-translate-y-1 transition-all duration-300 flex flex-col items-center justify-center gap-2 group"><span className="text-2xl md:text-3xl bg-white text-blue-600 p-2 md:p-3 rounded-2xl shadow-sm group-hover:scale-110 transition duration-300"><LuReceipt /></span><span className="font-bold text-blue-900 text-sm md:text-base">New Bill</span></Link>
            
            {/* 2. Inventory */}
            <Link href="/inventory" className="bg-emerald-50 border border-emerald-100 p-4 md:p-6 rounded-3xl shadow-sm hover:shadow-md hover:bg-emerald-100 hover:-translate-y-1 transition-all duration-300 flex flex-col items-center justify-center gap-2 group"><span className="text-2xl md:text-3xl bg-white text-emerald-600 p-2 md:p-3 rounded-2xl shadow-sm group-hover:scale-110 transition duration-300"><LuPill /></span><span className="font-bold text-emerald-900 text-sm md:text-base">Inventory</span></Link>
            
            {/* 3. Appointments */}
            <Link href="/admin/appointments" className="bg-violet-50 border border-violet-100 p-4 md:p-6 rounded-3xl shadow-sm hover:shadow-md hover:bg-violet-100 hover:-translate-y-1 transition-all duration-300 flex flex-col items-center justify-center gap-2 group"><span className="text-2xl md:text-3xl bg-white text-violet-600 p-2 md:p-3 rounded-2xl shadow-sm group-hover:scale-110 transition duration-300"><LuCalendar /></span><span className="font-bold text-violet-900 text-sm md:text-base">Appointments</span></Link>
            ...
            <Link href="/patient" className="bg-cyan-50 border border-cyan-100 p-4 md:p-6 rounded-3xl shadow-sm hover:shadow-md hover:bg-cyan-100 hover:-translate-y-1 transition-all duration-300 flex flex-col items-center justify-center gap-2 group">
                <span className="text-2xl md:text-3xl bg-white text-cyan-600 p-2 md:p-3 rounded-2xl shadow-sm group-hover:scale-110 transition duration-300"><LuUsers /></span>
            ...
            {userRole === 'admin' && (
                <Link href="/admin/sms" className="bg-rose-50 border border-rose-100 p-4 md:p-6 rounded-3xl shadow-sm hover:shadow-md hover:bg-rose-100 hover:-translate-y-1 transition-all duration-300 flex flex-col items-center justify-center gap-2 group">
                    <span className="text-2xl md:text-3xl bg-white text-rose-600 p-2 md:p-3 rounded-2xl shadow-sm group-hover:scale-110 transition duration-300"><LuSmartphone /></span>
        ...
        <div className={`grid grid-cols-1 lg:grid-cols-${userRole === 'admin' ? '2' : '1'} gap-6 md:gap-8`}>
            ...
            <div className="bg-white p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] shadow-sm border border-slate-200 h-full flex flex-col">
                <div className="flex justify-between items-center mb-6"><h3 className="text-lg md:text-xl font-bold text-slate-800 flex items-center gap-3"><span className="bg-slate-100 p-2 rounded-xl text-lg md:text-xl"><LuCalendarDays /></span> Holidays</h3></div>
                ...
                {userRole === 'admin' && (
                    <div className="bg-slate-50 p-2 rounded-2xl border border-slate-200 flex flex-wrap items-center gap-2 mb-6">
                        ...
                        <select className="bg-white p-2.5 md:p-3 rounded-xl border-none text-xs md:text-sm font-bold text-slate-800 outline-none focus:ring-2 focus:ring-blue-100 cursor-pointer flex-1 md:flex-none" value={holidaySession} onChange={e => setHolidaySession(e.target.value)}><option value="full">Full Day</option><option value="morning">Morning</option><option value="evening">Evening</option></select>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar min-h-[150px] pr-2">
                    {holidaysList.length === 0 ? <div className="h-full flex flex-col items-center justify-center text-slate-300 border-2 border-dashed border-slate-100 rounded-2xl p-6"><span className="text-4xl mb-3 text-slate-200"><LuUmbrella /></span><p className="text-sm font-bold">No upcoming holidays</p></div> : <div className="space-y-3">{holidaysList.map(h => (<div key={h.id} className="flex justify-between items-center bg-white p-3 md:p-4 rounded-2xl border border-slate-100 hover:border-slate-300 hover:shadow-sm transition group"><div className="flex items-center gap-3"><span className={`w-8 h-8 md:w-10 md:h-10 flex items-center justify-center rounded-full text-base md:text-lg ${h.type === 'poya' ? 'bg-yellow-100 text-yellow-600' : 'bg-red-100 text-red-600'}`}>{h.type === 'poya' ? <LuMoon className="w-5 h-5 fill-current" /> : <LuCircle className="w-5 h-5 fill-current text-red-500" />}</span><div><p className="text-sm font-bold text-slate-800">{h.date}</p><div className="flex gap-2 text-[10px] font-bold uppercase mt-0.5"><span className="text-slate-400">{h.type}</span>{h.session === 'morning' && <span className="text-orange-500 bg-orange-50 px-1.5 rounded flex items-center gap-1"><LuSun className="w-3 h-3" /> Morning Off</span>}{h.session === 'evening' && <span className="text-blue-500 bg-blue-50 px-1.5 rounded flex items-center gap-1"><LuMoon className="w-3 h-3" /> Evening Off</span>}{(!h.session || h.session === 'full') && <span className="text-slate-500 bg-slate-100 px-1.5 rounded flex items-center gap-1"><LuCalendarDays className="w-3 h-3" /> Full Day</span>}</div></div></div>{userRole === 'admin' && <button onClick={() => handleDeleteHoliday(h.id)} className="w-8 h-8 rounded-full bg-slate-50 text-slate-400 hover:bg-red-50 hover:text-red-500 flex items-center justify-center transition"><LuTrash className="w-4 h-4" /></button>}</div>))}</div>}
                </div>
            </div>
            ...
            {userRole === 'admin' && (
                <div className="bg-white p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] shadow-sm border border-slate-200 h-full flex flex-col relative overflow-hidden">
                    ...
                    <h3 className="text-lg md:text-xl font-bold text-slate-800 mb-6 md:mb-8 flex items-center gap-3 relative z-10"><span className="bg-slate-100 p-2 rounded-xl text-lg md:text-xl"><LuUser /></span> Staff Access <span className="text-[10px] bg-slate-800 text-white px-2 py-1 rounded-full ml-2">Max 3</span></h3>
